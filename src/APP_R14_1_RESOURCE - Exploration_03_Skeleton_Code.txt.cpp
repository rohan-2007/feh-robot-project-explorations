...
void check_heading(float heading)
{
    ...
    for (int i = 0; i < 10; i++) {
        while(pose != nullptr && (pose->heading < heading - 3 || pose->heading > heading + 3)) {
...
void ERCMain()
{
    ...
    fscanf(fptr, "%f%f", &A_x, &A_y);
    fscanf(fptr, "%f%f", &B_x, &B_y);
    fscanf(fptr, "%f%f", &C_x, &C_y);
    fscanf(fptr, "%f%f", &D_x, &D_y);
    fclose(fptr);
